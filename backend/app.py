...
def windows_thumbnail(path, size=256):
    """Return a PNG thumbnail generated by Windows shell or None on failure."""
    ...
    try:
        ...
        factory = shell.SHCreateItemFromParsingName(
            path, None, shell.IID_IShellItemImageFactory
        )
        hbitmap, _ = factory.GetImage((size, size), 0)
...
@app.route('/files/<int:file_id>/thumbnail', methods=['GET'])
def file_thumbnail(file_id):
    file = File.query.get_or_404(file_id)
    data = windows_thumbnail(file.path)
    if data:
